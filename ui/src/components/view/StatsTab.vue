...
<template>
  <div class="container">
    ...
    <a-row class="chart-row">
      <a-col>
        <span class="ant-tag">
          <a-button @click="showFilterStatsModal = true">
            <a-icon type="filter"/>
          </a-button>
    ...
    <div v-if="loaded">
      <div v-if="chartLabels.length > 0">
        <a-row class="chart-row">
          <a-col>
            <strong>CPU</strong>
            <a-icon class="info-icon" :title="$t('label.see.more.info.cpu.usage')" type="info-circle" @click="onClickShowResourceInfoModal('CPU')"/>
            <multi-line-chart
        ...
        <a-row class="chart-row">
          <a-col>
            ...
            <a-icon class="info-icon" :title="$t('label.see.more.info.network.usage')" type="info-circle" @click="onClickShowResourceInfoModal('NET')"/>
            <a-select v-model="selectedNetworkUnitOfMeasurement">
...
export default {
  ...
  methods: {
    ...
    getChartOptions (yAxesOptions, yAxesUnitOfMeasurement) {
      ...
      chartOptions.scales.xAxes[0].time.unitStepSize = this.calculateStepSize(this.chartLabels.length, averageDifference)
      return chartOptions
    },
