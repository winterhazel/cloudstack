...
public class KbossBackupProviderTest {
    ...
    public void createDeltaReferencesTestNotFullBackupEndOfChain() {
        ...
        doReturn(null).when(kbossBackupProviderSpy).createDeltaMergeTreeForVolume(eq(false), eq(true), any(), eq(null), any(), any());
    ...
    public void orchestrateTakeBackupTestIsolatedBackupFailed() {
        ...
        verify(kbossBackupProviderSpy, Mockito.times(2)).createDeltaReferences(Mockito.anyBoolean(), Mockito.anyBoolean(), any(), any(), any(), any(), any(), any(), eq(null), any());
    ...
    public void createDeltaMergeTreeTestChildIsDeltaWithSucceedingSnapshotReferences() {
        ...
        doReturn("child-path").when(snapshotRefMock).getInstallPath();
        doReturn(List.of(snapshotRefMock)).when(vmSnapshotHelperMock).getVolumeSnapshotsAssociatedWithKvmDiskOnlyVmSnapshot(anyLong());
        doReturn(new ArrayList<>()).when(vmSnapshotDaoMock).listByParent(anyLong());

        DeltaMergeTreeTO result = kbossBackupProviderSpy.createDeltaMergeTree(false, false, internalBackupStoragePoolVoMock,
                volumeObjectToMock, vmSnapshotVoMock, null);
